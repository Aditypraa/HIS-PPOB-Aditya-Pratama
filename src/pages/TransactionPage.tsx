...
function TransactionPage() {
    ...
    const [history, setHistory] = useState<Transaction[]>(state.transactionHistory.records);
    ...
    const fetchHistory = useCallback(async (currentOffset: number) => {
        ...
        try {
            const response = await apiService.getTransactionHistory(currentOffset, limit);
        ...
        <div className="min-h-screen bg-gray-100 pb-20">
            ...
            <main className="container mx-auto p-4">
                <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
                    <h2 className="text-xl font-semibold text-gray-800 mb-4">Semua Transaksi</h2>
                    ...
                    {history.length > 0 && (
                        <ul className="divide-y divide-gray-200">
                            {history.map((tx, index) => {
                                ...
                                return (
                                    <li key={`${tx.invoice_number}-${index}`} className="py-4 flex justify-between items-center">
                                        <div className="flex items-center">
                                            ...
                                            <div>
                                                <p className={`font-medium ${style.color}`}>{tx.description}</p>
                                                <p className="text-xs text-gray-500">{formatDate(tx.created_on)}</p>
                                                <p className="text-xs text-gray-400 mt-1">Inv: {tx.invoice_number}</p>
                                            </div>
                                        </div>
                                        <span className={`font-semibold text-lg ${style.color}`}>
                                            {style.sign}{formatCurrency(tx.total_amount)}
                                        </span>
                                    </li>
                                );
                            })}
                    ...
                    {!hasMore && history.length > 0 && (
                        <p className="text-center text-gray-400 mt-6 text-sm">-- Akhir Riwayat --</p>
                    )}
                </div>
            </main>
