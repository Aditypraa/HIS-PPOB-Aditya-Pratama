...
function PaymentPage() {
  ...
  const { user, balance, pageData, isSubmitting } = state;
  const [step, setStep] = useState("confirm");
  const {
    serviceCode = "UNKNOWN",
    serviceName = "Layanan Tidak Dikenal",
    amount = 0,
  } = pageData || {};
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      if (balance !== null && balance < amount) {
        throw { message: "Saldo tidak mencukupi." };
      } // Throw local error
      await apiService.createTransaction(serviceCode);
      setStep("success");
  ...
  const ServiceDisplayIcon = serviceCode
    ? getFallbackIcon(serviceCode)
    : CreditCard;
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl w-full max-w-md text-center relative">
        ...
        {(step === "success" || step === "failed") && !isSubmitting && (
          <button
          ...
          <p className="text-sm text-gray-600 mt-2">
            ...
            <span
              className={`font-semibold ml-1 ${
                balance === null ? "italic text-gray-400" : ""
              }`}
            >
              {" "}
              {balance === null ? "Memuat..." : formatCurrency(balance)}{" "}
            </span>{" "}
          </p>{" "}
        </div>
        {/* Payment Steps */}
        {step === "confirm" && (
          <>
            ...
            <p className="text-gray-600 mb-1">Anda akan melakukan pembayaran</p>{" "}
            <p className="text-lg font-medium text-gray-900 mb-1">
              {serviceName}
            </p>{" "}
            <p className="text-gray-600 mb-4">senilai</p>{" "}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Button
                ...
                disabled={isSubmitting || balance === null || balance < amount}
              >
                {" "}
        ...
        {step === "success" && (
          <>
            ...
            <p className="text-sm text-gray-500 mb-6">berhasil dilakukan.</p>{" "}
            <Button onClick={handleGoHome} fullWidth isLoading={isSubmitting}>
              {" "}
              Kembali ke Beranda{" "}
            </Button>{" "}
          </>
